...
const BlogSchema: Schema = new Schema({
    ...
    const Blog: any = this; // tslint:disable-line
    ...
    try {
        ...
        const hash: string = await bcrypt.hash(Blog.password, salt);
 ...
 */
BlogSchema.methods.comparePassword = async function (candidatePassword: string): Promise < boolean > {
    try {
 ...
 */
BlogSchema.methods.gravatar = function (size: number): string {
    if (!size) {
        size = 200;
    }
    if (!this.email) {
